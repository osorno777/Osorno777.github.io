...
- **Private HTML masters / rebuilds:** CC-AB-Store gates `/admin/translations/private/`. On this PC, hardened rebuild HTML is already at `C:\Alertness AI\bookstore\fulfillment\_out` (for example `btc5_rebuilds\btc-5_hi.html`). Those are scanned as HTML, separate from KDP PDFs and store EPUBs. Do not copy sidecar JSON.
