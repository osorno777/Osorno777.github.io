...
py -m translation_qa use-drive --letter E
py -m translation_qa compact-reports --output E:\translation_qa\reports
...
Reports go to `E:\translation_qa\reports` (compact HTML plus a small JSON summary). Temp and `__pycache__` go to `E:\translation_qa\tmp` and `pycache`. `.\rescan.bat` moves leftover `C:` copies there first so the BTC scan cannot fill `C:` again. Use `--full-reports` only if `E:` has spare space.
